...
# 复用 doc_loader 加载文档（从 shared/scripts/ 导入）
SHARED_SCRIPT_DIR = Path(__file__).resolve().parent.parent.parent.parent / "shared" / "scripts"
sys.path.insert(0, str(SHARED_SCRIPT_DIR))
from doc_loader import load_directory


def load_industry_profiles():
    """Load industry profiles from shared JSON file. Returns (metrics_dict, aliases_dict) or (None, None)."""
    try:
        profile_path = Path(__file__).resolve().parent.parent.parent.parent / "shared" / "references" / "industry-profiles.json"
